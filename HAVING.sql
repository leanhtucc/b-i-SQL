...
SELECT [SupplierID], SUM([UnitsInStock]) AS [SumProduct], AVG([UnitPrice]) AS [AVGProduct]
FROM [DBO].[Products]
GROUP BY [SupplierID]
...
SELECT [OrderID], SUM([Freight]) AS [SumFreight], MONTH([ShippedDate]) AS [MONTHOrder]
FROM [dbo].[Orders]
...
ORDER BY MONTH([ShippedDate]) ASC;
--Hãy lọc những thành phố có số lượng đơn hàng > 16 và sắp xếp theo tổng số lượng giảm dần 
